					...
					<script type="text/javascript">
						jQuery(document).ready(function() {
							...
							foreach ($this->actions as $action_name => $action) { ?>
								jQuery('<option>').val('<?php echo esc_html( $action_name ) ?>').text('<?php echo esc_html( $action["menu_text"] ) ?>').appendTo("select[name='action']");
								jQuery('<option>').val('<?php echo esc_html( $action_name ) ?>').text('<?php echo esc_html( $action["menu_text"] ) ?>').appendTo("select[name='action2']");

								jQuery('.search-box').append( '<a href="javascript:;" class="button button-primary process-payment all" style="margin-left: 10px;"><?php esc_html_e( "Process All Orders", "couponxxl" ) ?></a>' );;
							<?php } ?>
						});
					</script>
				<?php
