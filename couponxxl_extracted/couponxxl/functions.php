            ...
            <tr>
                ...
                <td>
                    <select name="cxxl_access_backend">
                        <option <?php echo 'yes' != $cxxl_access_backend ? 'selected="selected"' : '' ?>
            ...
            <tr>
                ...
                <td>
                    <select name="cxxl_account_type">
                        <option <?php echo 'vendor' == $cxxl_account_type ? 'selected="selected"' : '' ?>
                                value="vendor"><?php esc_html_e( 'Vendor', 'couponxxl' ) ?></option>
                        <option <?php echo 'agent' == $cxxl_account_type ? 'selected="selected"' : '' ?>
                                value="agent"><?php esc_html_e( 'Vendor Agent', 'couponxxl' ) ?></option>
                        <option <?php echo 'buyer' == $cxxl_account_type ? 'selected="selected"' : '' ?>
...
if ( ! function_exists( 'couponxxl_scripts_styles' ) ) {
	function couponxxl_scripts_styles() {
		...
		if ( ! empty( $theme_font ) ) {
			wp_enqueue_style( 'couponxxl-theme-font', 'https:' . couponxxl_load_google_font( $theme_font ), array(), '1.0.0' );
		}

		// Load additional fonts if they're different from theme_font
		$text_font = couponxxl_get_option( 'text_font' );
		if ( ! empty( $text_font ) && $text_font !== $theme_font ) {
			wp_enqueue_style( 'couponxxl-text-font', 'https:' . couponxxl_load_google_font( $text_font ), array(), '1.0.0' );
		}

		$titles_font = couponxxl_get_option( 'titles_font' );
		if ( ! empty( $titles_font ) && $titles_font !== $theme_font && $titles_font !== $text_font ) {
			wp_enqueue_style( 'couponxxl-titles-font', 'https:' . couponxxl_load_google_font( $titles_font ), array(), '1.0.0' );
		}

		$navigation_font = couponxxl_get_option( 'navigation_font' );
		if ( ! empty( $navigation_font ) && $navigation_font !== $theme_font && $navigation_font !== $text_font && $navigation_font !== $titles_font ) {
			wp_enqueue_style( 'couponxxl-navigation-font', 'https:' . couponxxl_load_google_font( $navigation_font ), array(), '1.0.0' );
		}
...
if ( ! function_exists( 'couponxxl_admin_resources' ) ) {
	function couponxxl_admin_resources() {
		...
		wp_enqueue_style( 'couponxxl-jquery-ui', $protocol . '://code.jquery.com/ui/1.11.2/themes/smoothness/jquery-ui.css' );
		wp_enqueue_script( 'jquery-ui-sortable' );
...
if ( ! function_exists( 'couponxxl_get_users_select' ) ) {
	function couponxxl_get_users_select() {
		...
		$options   = wp_cache_get( $cache_key, 'couponxxl-user-field' );
		if ( false !== $options ) {
			return $options;
		}
		...
		foreach ( $users as $user ) {
			...
			$options[ $user->ID ] = $user_data->user_nicename;
		}

		// Cache the query.
		wp_cache_set( $cache_key, $options, 'couponxxl-user-field' );
...
if ( ! function_exists( 'couponxxl_comments' ) ) {
	function couponxxl_comments( $comment, $args, $depth ) {
		...
		?>
        <!-- comment-1 -->
        <div class="media <?php echo 1 < $depth ? esc_attr( 'left-padding' ) : esc_attr( '' ); ?>">
            <div class="comment-inner">
                ...
                <div class="media-body comment-body">
                    <div class="pull-left">
                        ...
                        <span><?php echo human_time_diff( get_comment_date( 'U' ), current_time( 'timestamp' ) ) . esc_html__( ' ago', 'couponxxl' ); ?></span>
                    </div>
...
if ( ! function_exists( 'couponxxl_show_code' ) ) {
	function couponxxl_show_code() {
		...
		if ( ! empty( $offer ) ) {
            ...
            <h4><?php echo wp_kses_post( $offer->post_title ); ?></h4>
			<?php
			if ( $coupon_type == 'code' ) {
...
if ( ! function_exists( 'couponxxl_kses' ) ) {
	function couponxxl_kses( $string ) {
		// iframe
