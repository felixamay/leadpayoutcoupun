...
if (0 == $offer_submit_price || $cxxl_credits >= $offer_submit_price || $edit):
    ...
    if ( ! empty($offer_submit_price) && ! $edit) {
        echo '<div class="alert alert-info">'.esc_html__('Submission of the',
                'couponxxl').' '.$name.' '.esc_html__('is charged',
                'couponxxl').' '.$offer_submit_price.' '.($offer_submit_price == '1' ? esc_html__('credit',
                'couponxxl') : esc_html__('credits', 'couponxxl')).'</div>';
    }
    ?>
    <?php if (in_array($offer_id, $offer_ids) || ! $edit) : ?>
    <form method="post">
        <div id="wizard" class="<?php echo 'coupon' == $offer_type ? esc_attr('coupon-wizard') : esc_attr('') ?>">
            <h1></h1>
            <div>
                <div class="input-group">
                    <label for="offer_title"><?php esc_html_e('Offer Title', 'couponxxl'); ?> <span
                                class="required">*</span></label>
                    <input type="text" name="offer_title" id="offer_title"
                           value="<?php echo esc_attr($offer_title) ?>" class="form-control"
                           data-validation="required"
                           data-error="<?php _e('Please input offer title', 'couponxxl'); ?>">
                    <p class="description"><?php esc_html_e('Input title for the offer.', 'couponxxl'); ?></p>
                </div>

                <label for="offer_description"
                       data-error="<?php _e('Please type description of the offer',
                           'couponxxl'); ?>"><?php esc_html_e('Offer Description', 'couponxxl'); ?>
                    <span class="required">*</span></label>
                <textarea type="text" name="offer_description" id="offer_description"
                          value="<?php echo esc_attr($offer_description) ?>" class="form-control"
                          style="padding-left: 20px;"
                          data-validation="required"
                          data-error="<?php _e('Please input offer description', 'couponxxl'); ?>"></textarea>
                <!--				--><?php //if ( pbs_is_demo() ) : ?>
                ...
                <div class="input-group">
                    <label for="offer_featured_image"><?php esc_html_e('Offer Image', 'couponxxl'); ?> <span
                                class="required">*</span></label>
                    <?php if (pbs_is_demo()) : ?>
                        <input type="hidden" name="offer_featured_image" id="offer_featured_image" value="">
                    <?php else: ?>
                        <input type="hidden" name="offer_featured_image" id="offer_featured_image"
                               value="<?php echo esc_attr($offer_featured_image) ?>" data-validation="required"
                               data-error="<?php _e('Please input offer presentation image', 'couponxxl'); ?>">
                    <?php endif; ?>
                    ...
                    <a href="javascript:;"
                       class="image-upload <?php echo pbs_is_demo() ? '' : esc_attr('featured-image'); ?>"><?php esc_html_e('Select featured image',
                            'couponxxl') ?></a>
                    <p class="description"><?php esc_html_e('Upload and select featured image for the offer.',
                            'couponxxl'); ?></p>
                </div>
            </div>
            ...
            <div>
                <div class="input-group">
                    <label for="offer_cat"><?php esc_html_e('Offer Category', 'couponxxl'); ?> <span
                                class="required">*</span></label>
                    <select name="offer_cat" id="offer_cat" class="form-control" data-validation="conditional"
                            data-conditional-field="offer_new_category"
                            data-error="<?php _e('Please select offer category', 'couponxxl'); ?>">
                    ...
                    <p class="description"><?php esc_html_e('Select category for the offer or populate field bellow to request new one leaving this field not selected.',
                            'couponxxl'); ?></p>
                </div>
                <div class="input-group">
                    <label for="offer_new_category"><?php esc_html_e('Category not listed?', 'couponxxl'); ?></label>
                    <textarea name="offer_new_category" id="offer_new_category"
                              class="form-control"><?php echo wp_kses_post($offer_new_category); ?></textarea>
                    <p class="description"><?php esc_html_e('Populate this field if desired category is not listed in the previous field.',
                            'couponxxl'); ?></p>
                </div>
            </div>
            ...
            <?php if ( ! $edit): ?>
                ...
                <div>
                    <div class="input-group">
                        ...
                        <input type="text" name="offer_start" id="offer_start"
                               value="<?php echo date('Y-m-d', $offer_start) ?>" class="form-control"
                               readonly="readonly" data-range="<?php echo esc_attr($date_ranges); ?>">
                        <p class="description"><?php esc_html_e('Set start date for the offer. If this field is empty current time will be applied to the offer.',
                                'couponxxl'); ?></p>
                    </div>

                    <div class="input-group">
                        <label for="offer_expire"><?php esc_html_e('Offer Expire Date', 'couponxxl'); ?> <span
                                    class="required">*</span></label>
                        <input type="text" name="offer_expire" id="offer_expire"
                               value="<?php echo ! empty($offer_expire) && $offer_expire !== '99999999999' ? date('Y-m-d',
                                   $offer_expire) : '' ?>"
                               class="form-control" readonly="readonly"
                               data-range="<?php echo esc_attr($date_ranges); ?>" <?php echo ! empty($date_ranges) ? 'data-validation="required"  data-error="'.esc_attr__('Please input offer expiration date',
                                'couponxxl').'"' : ''; ?> >
...
<?php else : ?>
    <div class="alert alert-danger"><?php esc_html_e('You\'re trying to reach things you shouldn\'t!',
            'couponxxl'); ?></div>
<?php endif; ?>
    <?php
    if ($edit) {
    ...
    ?>
<?php else: ?>
    <div class="alert alert-danger"><?php esc_html_e('You do not have enough credits on your account to submit this offer',
            'couponxxl') ?></div>
<?php endif; ?>