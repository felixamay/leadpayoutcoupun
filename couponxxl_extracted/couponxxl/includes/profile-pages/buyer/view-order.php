...
if( !empty( $order ) ){
	...
	if( $order->post_author == get_current_user_id() && $order_status == 'paid' ){
		?>
		<h4><strong><?php echo wp_kses_post( $order->post_title ); ?></strong></h4>
				...
				<table>
					...
					if( !empty( $order_items ) ){
						...
						foreach( $order_items as $order_item ){
							...
							?>
							<tr>
								<td class="offer-title"><?php echo wp_kses_post( $order_item->offer_title ); ?></td>
								...
								<div class="voucher-code">
									...
									if( empty( $order_item->status )  ){
										...
										echo esc_html( $order_item->voucher_code );
										$can_use = true;
									}
									?>
								</div>
