...
if ( ! function_exists( 'couponxxl_get_the_offer_type' ) ) {
	function couponxxl_get_the_offer_type( $post = 0 ) {
		$post            = get_post( $post );
		$offer_type_meta = couponxxl_get_post_meta( $post->ID, 'offer_type', true ) ? couponxxl_get_post_meta( $post->ID, 'offer_type', true ) : '';
		$offer_type      = isset( $post->offer_type ) ? $post->offer_type : $offer_type_meta;
	...
	function couponxxl_offer_countdown( $offer_expire ) {
        ...
        <div class="countdown">
			...
			<?php if ( $offer_expire !== '99999999999' ) : ?>
                <span class="deal-countdown" data-single="<?php _e( 'Day', 'couponxxl' ) ?>"
                      data-multiple="<?php _e( 'Days', 'couponxxl' ) ?>"
                      data-expire="<?php echo esc_attr( $offer_expire ) ?>"
...
if ( ! function_exists( 'couponxxl_quantity_input' ) ) {
	function couponxxl_quantity_input( $post_id = '', $value = '' ) {
		...
		$deal_items = empty( $deal_items ) ? 0 : $deal_items;
		$deal_sales = empty( $deal_sales ) ? 0 : $deal_sales;
		$available  = $deal_items - $deal_sales;

		return '
	<input type="number" min="1" max="' . esc_attr( $available ) . '" name="qty_' . ( esc_attr( $post_id ) ) . '" value="' . esc_attr( $value ) . '"/>
...
if ( ! function_exists( 'couponxxl_add_to_cart' ) ) {
	function couponxxl_add_to_cart( $text, $post_id = '' ) {
		...
		if ( empty( $account_type ) || $account_type == 'buyer' ) {
			...
			?>
            <a href="javascript:;" class="btn add-to-cart" data-id="<?php echo esc_attr( $post_id ) ?>"
               title="<?php _e( 'Add To Cart', 'couponxxl' ); ?>"><?php echo wp_kses_post( $text ); ?></a>
			<?php
		}
	}
}
