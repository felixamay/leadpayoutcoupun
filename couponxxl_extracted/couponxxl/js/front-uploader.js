jQuery(document).ready(function($){
	...
	$(document).on('click', '#wizard ul[aria-label="Pagination"] a', function(){
		if( $('.deal-images-wrap').length > 0 ){
	...
	/* CHANGE STORE LOGO */
	$(document).on( 'click', '.store-logo', function(e){
		var $this = $(this);
		e.preventDefault();
		...
		handle_images( frameArgs, function( selection ){
			...
			$.ajax({
				...
				method: "POST",
				data:{
					action: 'update_store_logo',
					image_id: model.id,
					store_id: $this.data('store_id')
				},
				success: function(response){
					$('.store-logo').html( response );
				}
			});
		});
	});	
